...
<script>
        const lang = {
            search_and_select_account: "{{ __('messages.search_and_select_account') }}",
            loading_details: "{{ __('messages.loading_details') }}",
            failed_to_load_details: "{{ __('messages.failed_to_load_details') }}",
            loan_details: "{{ __('messages.loan_details') }}",
            payable: "{{ __('messages.payable') }}",
            paid: "{{ __('messages.paid') }}",
            due: "{{ __('messages.due') }}",
            installment: "{{ __('messages.installment') }}",
        };
    </script>
<script>
        $(document).ready(function() {
            ...
            loanAccountSelect.select2({
                placeholder: lang.search_and_select_account,
                width: '100%',
            });
            ...
            loanAccountSelect.on('change', function() {
                ...
                if (!accountId) {
                    summaryContent.html(`<p class="text-muted">${lang.select_account}</p>`);
                    return;
                }
                summaryContent.html(`<div class="spinner-border spinner-border-sm"></div> ${lang.loading_details}`);
                    ...
                    success: function(response) {
                        ...
                        let html = `
                            ...
                            <p class="text-muted text-center small mb-3">
                                ...
                                <i data-lucide="map-pin" class="icon-sm me-1"></i> ${member.present_address}
                            </p>
                            <hr>
                            <h6 class="mb-3">${lang.loan_details}</h6>
                            <dl class="row small">
                                <dt class="col-sm-5">${lang.payable}</dt><dd class="col-sm-7">: ${parseFloat(response.total_payable).toFixed(2)}</dd>
                                <dt class="col-sm-5">${lang.paid}</dt><dd class="col-sm-7">: ${parseFloat(response.total_paid).toFixed(2)}</dd>
                                <dt class="col-sm-5">${lang.due}</dt><dd class="col-sm-7 fw-bold text-danger">: ${due.toFixed(2)}</dd>
